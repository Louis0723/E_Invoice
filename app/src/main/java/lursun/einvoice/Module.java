            ...
            public void run() {
                ...
                if (!(type == null || type.equals(""))) {
                    ...
                    if(type.equals("明細")){
                        switch (machine) {
                            case "頂尖":
                                byte[] _ESCW1 = {(byte) 0x1B, 'W', 0x00, 0x00, 0x00, 0x00, (byte) 0xC8, (byte) 0x01, (byte) 0x80, (byte) 0x02};
                                epson.ESCW = _ESCW1;
                                epson.initPrint();
                                epson.SetXY(18 - title.getBytes().length / 2, 10);
                            ...
                            case "BPT3":
                                byte[] _ESCW2 = {(byte) 0x1B, 'W', 0x00, 0x00, 0x00, 0x00, (byte) 0xC8, (byte) 0x01, (byte) 0x00, (byte) 0x02};
                                epson.ESCW = _ESCW2;
                                epson.initPrint();
                                epson.SetXY(20 - title.getBytes().length / 2, 0);
                                epson.Big(String.format("%s", title));
                                epson.SetXY(20 - "交易明細".getBytes().length / 2, 6);
                                epson.Big(String.format("交易明細"));
                                epson.SetXY(6, 13);
                                epson.Nomal(String.format("%s", date));
                                epson.SetXY(6, 17);
                        ...
                        switch (machine) {
                            case "頂尖":
                                epson.initPrint();
                                epson.SetXY((int) (19 - (float) title.getBytes().length / 1.5), 10);
                                epson.Big(String.format("%s", title));
                                epson.SetXY(type.equals("補印") ?0:4, 16);
